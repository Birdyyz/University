...
def vectorize_text(text, word_index, max_words): 
    ## one hot encoding (simply presence/ absence of word as feature) [0, 1, 0, 1]
...
def encode(text, word_index, max_len=200): ## integer encoding [2, 3, 1]
    tokens = clean_text(text).split()
    
    sequence = [word_index.get(word, 1) for word in tokens] # if word is unknown: 1 
    # Truncate
    sequence = sequence[:max_len]
    # Pad (works like a filler to keep the same size ex: [2,3,1,0,0,0,0])
...
def test_lstm():
    ...
    embed_dim = 200
    train_loader, val_loader, test_loader = load_dataset_embed(filespath, max_words, max_len = max_len)
    model = LSTMClassifier(max_words,embed_dim, hidden_dim=128, num_layers=1, bidirectional = True, dropout = 0.2)
    criterion = nn.BCEWithLogitsLoss()
